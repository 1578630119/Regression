...
def stand_regres(x_arr, y_arr):
    ...
    if np.linalg.det(xTx) == 0.0:  #判断矩阵的逆是否存在
...
def lwlr(test_point, x_arr, y_arr, k=1.0):
    ...
    for j in range(m):
        ...
        weights[j, j] = np.exp(diff_mat * diff_mat.T / (-2.0 * k**2))  #高斯核
...
'''def set_data_collect(ret_x, ret_y):
    scrapePage(ret_x, ret_y, 'Dataset/setHtml/lego8288.html', 2006, 800, 49.99)
    scrapePage(ret_x, ret_y, 'Dataset/setHtml/lego10030.html', 2002, 3096, 269.99)
    scrapePage(ret_x, ret_y, 'Dataset/setHtml/lego10179.html', 2007, 5195, 499.99)
    scrapePage(ret_x, ret_y, 'Dataset/setHtml/lego10181.html', 2007, 3428, 199.99)
    scrapePage(ret_x, ret_y, 'Dataset/setHtml/lego10189.html', 2008, 5922, 299.99)
    scrapePage(ret_x, ret_y, 'Dataset/setHtml/lego10196.html', 2009, 3263, 249.99)'''
...
if __name__ == "__main__":
    ...
    # 1.1 计算回归系数
    '''
    x_arr, y_arr = load_data_set('Dataset/ex0.txt')
    ...
    # 3. 示例：预测鲍鱼的年龄
    # 3.1 线性回归和LWLR
    ab_x, ab_y = load_data_set('Dataset/abalone.txt')
